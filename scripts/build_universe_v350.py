...
common equity. This wrapper rejects generic non-ETF funds and known CEF-sponsor
trust products before delegating every other rule to the pinned core builder.
"""
...
def classify_row(row, include_etfs, approved_etfs=None):
    ...
    if not is_etf and (_GENERIC_NON_ETF_FUND.search(name) or _CEF_SPONSOR_TRUST.search(name)):
