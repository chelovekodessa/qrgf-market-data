...
def validate_campaign_state(value:Mapping[str,Any],master:Mapping[str,Any])->dict[str,Any]:
    ...
    if v["phase"]=="COMPLETE" and not (v.get("daily_broad_allowed") is True and int(v.get("master_durable_count"))==MASTER_SIZE):raise ValueError("invalid V4.1 COMPLETE state")
    return v
def rebuild_campaign_state(root:Path,release_sha:str,published_at:str)->dict[str,Any]|None:
    ...
    phase="COMPLETE" if total==MASTER_SIZE else "CORE500" if pilot_ok else "PILOT" if runtime_ok else "CANARY"
